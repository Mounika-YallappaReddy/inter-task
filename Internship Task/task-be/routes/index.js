...
router.post('/signup',async(req,res)=>{
  ...
  let query =await connection.query("INSERT INTO loginapp.users(FIRSTNAME,LASTNAME,EMAIL,AGE,MOBILE,USERPASSWORD) VALUES(?,?,?,?,?,?)",[firstname,lastname,email,age,mobile,password],(err,result)=>{
    if(err) throw err;
   ...
   router.post('/signin',async(req,res)=>{
    ...
    const email=req.body.email;
    const password=req.body.password;
    let query =await connection.query("select * from loginapp.users where email=? and userpassword=?",[email,password],(err,result)=>{
      if(err){
        res.send({
          err:err
        })}
